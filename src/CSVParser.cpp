...
int CSVParser::parseInt(const string& text, int lineNumber, const string& fieldName) {
    const string trimmed = trim(text);
    if (trimmed.empty()) {
        throw runtime_error("Invalid integer value for " + fieldName + " at line " + to_string(lineNumber));
    }
    try {
        size_t pos = 0;
        ...
        throw runtime_error("Invalid integer value for " + fieldName + " at line " + to_string(lineNumber) + ": '" + trimmed + "'");
    }
}
...
vector<Event> CSVParser::parse(const string& path) {
    ...
    while (getline(input, line)) {
        ...
        if (columns.size() != 5) {
            throw runtime_error("Invalid CSV format at line " + to_string(lineNumber) + ": expected 5 columns, got " + to_string(columns.size()));
        }
        ...
        if (event.action != "request" && event.action != "release") {
            throw runtime_error("Invalid action at line " + to_string(lineNumber) + ": must be 'request' or 'release'");
        }
        if (event.duration < 0) {
            throw runtime_error("Invalid duration at line " + to_string(lineNumber) + ": must be >= 0");
        }
