...
int WFGraph_findPid(WFGraph *graph, const char *pid) {
  for (int i = 0; i < graph->pidCount; i++) {
    if (strcmp(graph->pidList[i], pid) == 0)
      return i;
  }
...
void WFGraph_removeEdge(WFGraph *graph, const char *wait, const char *hold) {
...
static bool dfsReach(WFGraph *graph, int cur, int target, bool *visited) {
  ...
  for (int i = 0; i < graph->adj[cur].size; i++) {
    ...
    if (!visited[next] && dfsReach(graph, next, target, visited))
      return true;
  }
...
bool WFGraph_pidInCycle(WFGraph *graph, const char *pid) {
  ...
  for (int i = 0; i < graph->adj[idx].size; i++) {
    ...
    bool found = dfsReach(graph, graph->adj[idx].data[i], idx, visited);
...
void DeadlockDetector_addWaitRelation(DeadlockDetector *detector,
                                      const char *waitingPid,
                                      const char *holdingPid) {
  WFGraph_addEdge(&detector->graph, waitingPid, holdingPid);
}

void DeadlockDetector_removeWaitRelation(DeadlockDetector *detector,
                                         const char *waitingPid,
                                         const char *holdingPid) {
  WFGraph_removeEdge(&detector->graph, waitingPid, holdingPid);
}

void DeadlockDetector_removeWaitingProcess(DeadlockDetector *detector,
                                           const char *pid) {
  WFGraph_removeOutgoingEdge(&detector->graph, pid);
}

void DeadlockDetector_removeProcess(DeadlockDetector *detector,
                                    const char *pid) {
...
bool DeadlockDetector_isInDeadlock(DeadlockDetector *detector,
                                   const char *pid) {
