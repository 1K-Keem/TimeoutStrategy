...
Resource &SimulationEngine::ensureResourceExists(const string &resourceId)
{
    ...
    {
        auto [newIt, inserted] = resources_.emplace(resourceId, Resource{resourceId});
        it = newIt;
    }
    return it->second;
}
...
SimulationMetrics SimulationEngine::run(const vector<Event> &sortedEvents)
{
    ...
    while (true)
    {
        ...
        // Xử lý sự kiện mới tại thời điểm hiện tại (nếu có request thì cấp phát tài nguyên luôn nếu có)
        processEventsAt(currentTime, sortedEvents, nextEventIndex);

        // Áp dụng chính sách timeout
        applyTimeouts(currentTime);

        // Cấp phát lại sau khi timeout (có thể có tài nguyên được giải phóng sau khi kill hoặc retry)
        grantPendingRequests(currentTime);

        // Quét để tìm các process đã hoàn thành
        checkAndCompleteProcesses();

        if (nextEventIndex >= sortedEvents.size() && pendingRequests_.empty() && !hasFutureRelease(currentTime))
        {
            break;
        }
...
void SimulationEngine::processEventsAt(int currentTime, const vector<Event> &events, size_t &nextEventIndex)
{
    while (nextEventIndex < events.size() && events[nextEventIndex].time == currentTime)
    {
        ...
        if (event.action == "request")
        {
            if (resource.isFree())
            {
                PendingRequest request{event.processId, event.resourceId, currentTime, event.duration, 0};
...
void SimulationEngine::releaseExpiredResources(int currentTime)
{
    ...
    {
        if (resource.releaseTime.has_value() && *resource.releaseTime <= currentTime)
        {
            resourcesToRelease.push_back(resourceId);
        }
    }

    // Release riêng từng resource sau khi đã thu thập hết để tránh sửa đổi iterator map
...
void SimulationEngine::grantPendingRequests(int currentTime)
{
    pendingRequests_.erase(
        remove_if(pendingRequests_.begin(), pendingRequests_.end(), [&](PendingRequest &request)
                  {
...
{
    const auto records = timeoutManager_.checkTimeouts(currentTime, processes_, resources_, pendingRequests_);
...
void SimulationEngine::allocateResource(Process &process, PendingRequest &request, int currentTime)
{
    auto &resource = ensureResourceExists(request.resourceId);
...
{
    if (process.state == ProcessState::Completed || process.state == ProcessState::Terminated)
    {
        return;
    }
...
void SimulationEngine::checkAndCompleteProcesses()
{
    for (auto &[id, process] : processes_)
    {
        if (process.state == ProcessState::Running &&
            process.heldResources.empty() &&
            remainingEventCount_[id] == 0 &&
...
void SimulationEngine::blockProcess(Process &process, const Event &event, int currentTime)
{
    process.state = ProcessState::Blocked;
    process.requestTime = currentTime;
    process.waitingFor = event.resourceId;
    pendingRequests_.push_back(PendingRequest{event.processId, event.resourceId, currentTime, event.duration, 0});
}

// Kiểm tra xem có tài nguyên nào sẽ được giải phong trong tương lai không (để check điều kiện dừng)
bool SimulationEngine::hasFutureRelease(int currentTime) const
{
    return any_of(resources_.begin(), resources_.end(), [&](const auto &pair)
        ...
        return resource.releaseTime.has_value() && *resource.releaseTime > currentTime; });
}
