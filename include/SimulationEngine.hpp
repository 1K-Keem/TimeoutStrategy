...
private:
    ...
    void processEventsAt(int currentTime, const vector<Event>& events, size_t& nextEventIndex);
    void releaseExpiredResources(int currentTime);
    void grantPendingRequests(int currentTime);
    void applyTimeouts(int currentTime);

    void allocateResource(Process& process, PendingRequest& request, int currentTime);
