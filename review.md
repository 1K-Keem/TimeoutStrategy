...
| --- | --- | --- |
| Models | `include/Models.h` | Kiểu dữ liệu chung. |
| CSVParser | `src/CSVParser.c` | Đọc + validate dataset, sort theo `time`. |
| SimulationEngine | `src/SimulationEngine.c` | Đồng hồ logic, event loop, cấp/giải phóng resource, gom metrics. |
| TimeoutManager | `src/TimeoutManager.c` | Tính `waiting_time`, thực thi kill / retry / rollback. |
| DeadlockDetector | `src/DeadlockDetector.c` | Wait-For Graph + DFS phát hiện chu trình. |
| main | `src/main.c` | CLI: parse args → chạy engine → in metrics. |
...
`TimeoutManager_checkTimeouts` duyệt mảng `pendingRequests`, chỉ xét process `PROCESS_STATE_BLOCKED`, tính `waitingTime = currentTime - request.requestTime`, trigger khi `>= timeout`. `TimeoutManager_init` in lỗi rồi `exit(1)` nếu `timeout < 1`.

Ground truth deadlock: `DeadlockDetector_isInDeadlock(detector, pid)` kiểm tra **đúng process bị timeout có nằm trong chu trình** (reachability `pid → ... → pid`), không dùng cycle toàn cục — nhờ vậy false positive đúng định nghĩa đề.

### 3.1 Kill (`doKillProcess`)

State → `PROCESS_STATE_TERMINATED`, giải phóng toàn bộ `heldResources`, xóa mọi pending của process. Quyết liệt nhất, throughput của process đó = 0.

### 3.2 Retry (`doRetryRequest`)

Gỡ request, `savedRetryCount = retryCount + 1`. Vượt `maxRetries` → leo thang kill. Ngược lại → `PROCESS_STATE_RUNNING`, hẹn `retryAfter = currentTime + retryDelay`, đẩy lại đúng request đang chờ với `requestTime = retryAfter`. Chỉ xin lại tài nguyên đang chờ, không trả lại tài nguyên đang giữ.

### 3.3 Rollback (`doRollbackProcess`)
...
Engine bắt record này trong `applyTimeouts` và gọi `SimulationEngine_replayProcess`: **re-inject toàn bộ request event** của process vào hàng đợi với `requestTime = currentTime`, reset `remainingEventCount` của process.
...
Để hỗ trợ rollback, engine lưu `processEvents_` (bảng process → `EventVector`) khi `SimulationEngine_registerEventSources`, dùng làm nguồn replay. Đây là thay đổi kiến trúc chính so với bản kill/retry.
...
- `WFGraph_deadlockDetection(graph)` — DFS bắt chu trình bất kỳ.
- `WFGraph_pidInCycle(graph, pid)` — reachability xác định process có thực sự trong chu trình.
...
Mỗi `TimeoutRecord`: `timeoutEvents++`; `falsePositives++` nếu process không trong chu trình; `retryEvents`/`rollbackEvents`/`killedProcesses` theo loại; `deadlockResolved++` chỉ khi process thực sự deadlock và đồ thị **có chu trình trước → hết chu trình sau** khi xử lý (tránh đếm trùng khi nhiều process trong cùng chu trình bị xử lý liên tiếp).
...
- **deadlock_resolved sai**: trước đếm theo mỗi record có cycle. Sửa: chỉ tăng khi "có chu trình → hết chu trình" quanh lúc xử lý (tránh đếm trùng khi nhiều process trong cùng một chu trình bị xử lý liên tiếp).
...
`tests/test_timeout.c` — harness tự viết (không framework ngoài). **61 checks, 0 fail**. Phủ:
